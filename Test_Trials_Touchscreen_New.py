...
expName = 'Test_Trials_Touchscreen'  # from the Builder filename that created this script
...
trials_2 = data.TrialHandler(nReps=4, method='random', 
    ...
    trialList=data.importConditions('testFile_v1_touchscreen_6_items_test.xlsx'),
...
#    second_image.opacity=0.0
    ...
    trialComponents = [first_image, text, mouse, Top_Left, Top_Middle, Top_Right, Right_Middle, Bottom_Right, Bottom_Middle, Bottom_Left, Left_Middle, invisible_clickable, green_check]
...
#            second_image.setAutoDraw(False)
        ...
        if Top_Left.contains(mouse):
            mouse.setPos(newPos=(0,0))
            thisExp.addData('Clicked','Top_Left')
            thisExp.addData('RT',t)
            continueRoutine = False
            
        if Top_Middle.contains(mouse):
            mouse.setPos(newPos=(0,0))
            thisExp.addData('Clicked','Top_Middle')
            thisExp.addData('RT',t)
            continueRoutine = False
            
        if Top_Right.contains(mouse):
            mouse.setPos(newPos=(0,0))
            thisExp.addData('Clicked','Top_Right')
            thisExp.addData('RT',t)
            continueRoutine = False
            
        if Right_Middle.contains(mouse):
            mouse.setPos(newPos=(0,0))
            thisExp.addData('Clicked','Right_Middle')
            thisExp.addData('RT',t)
            continueRoutine = False
            
        if Bottom_Right.contains(mouse):
            mouse.setPos(newPos=(0,0))
            thisExp.addData('Clicked','Bottom_Right')
            thisExp.addData('RT',t)
            continueRoutine = False
            
        if Bottom_Middle.contains(mouse):
            mouse.setPos(newPos=(0,0))
            thisExp.addData('Clicked','Bottom_Middle')
            thisExp.addData('RT',t)
            continueRoutine = False
            
        if Bottom_Left.contains(mouse):
            mouse.setPos(newPos=(0,0))
            thisExp.addData('Clicked','Bottom_Left')
            thisExp.addData('RT',t)
            continueRoutine = False
            
        if Left_Middle.contains(mouse):
            mouse.setPos(newPos=(0,0))
            thisExp.addData('Clicked','Left_Middle')
            thisExp.addData('RT',t)
            continueRoutine = False
            ...
            if buttons != prevButtonState:  # button state changed?
                ...
                if sum(buttons) > 0:  # state changed to a new click
                   ...
                   # mouse.x.append(x)
                    ...
                    for obj in [Top_Left,Top_Middle,Top_Right,Right_Middle,Bottom_Right,Bottom_Middle,Bottom_Left,Left_Middle]:
