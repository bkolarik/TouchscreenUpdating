...
expName = 'Learn_Trials_Touchscreen'  # from the Builder filename that created this script
...
green_check = visual.ImageStim(
    ...
    ori=0, pos=(0.54, 0.54), size=(0.3, 0.3),
    color=[1,1,1], colorSpace='rgb', opacity=0,
    flipHoriz=False, flipVert=False,
...
trials_2 = data.TrialHandler(nReps=12, method='random', 
    ...
    trialList=data.importConditions('testFile_v1_touchscreen_6_items.xlsx'),
...
for thisTrial_2 in trials_2:
    ...
    pause_now = False    # update component parameters for each repeat
    first_image.setPos(placement1)
    first_image.setImage(stimImg_1)
    second_image.setPos(placement2)
    ...
    invisible_clickable.setPos(placement2)
    ...
    while continueRoutine:
        ...
        if second_image.opacity == 0.0:
            ...
            thisExp.addData('Correct Trial',1)

        if t >= 5:
            second_image.opacity=1.0
            thisExp.addData('Correct Trial',0)
        ...
        if np.all([Top_Left.status == STARTED,second_image.opacity>0.0,bool(Original=='Top_Left')]):
        ...
        if np.all([Top_Middle.status == STARTED,second_image.opacity>0.0,bool(Original=='Top_Middle')]):
        ...
        if np.all([Top_Right.status == STARTED,second_image.opacity>0.0,bool(Original=='Top_Right')]):
        ...
        if np.all([Right_Middle.status == STARTED,second_image.opacity>0.0,bool(Original=='Right_Middle')]):
        ...
        if np.all([Bottom_Right.status == STARTED,second_image.opacity>0.0,bool(Original=='Bottom_Right')]):
        ...
        if np.all([Bottom_Middle.status == STARTED,second_image.opacity>0.0,bool(Original=='Bottom_Middle')]):
        ...
        if np.all([Bottom_Left.status == STARTED,second_image.opacity>0.0,bool(Original=='Bottom_Left')]):
        ...
        if np.all([Left_Middle.status == STARTED,second_image.opacity>0.0,bool(Original=='Left_Middle')]):
